...
class Map {
    ...
    function get_location($query)
    {
		...
		$adminDistrict = $xmlResponse->ResourceSets->ResourceSet->Resources->Location->Address->AdminDistrict;
		$adminDistrictSub = $xmlResponse->ResourceSets->ResourceSet->Resources->Location->Address->AdminDistrict2;
		$country = $xmlResponse->ResourceSets->ResourceSet->Resources->Location->Address->CountryRegion;
		
		$result = array(
				"name" => $formattedName,
				"admin_district" => $adminDistrict,
				"admin_district_sub" => $adminDistrictSub,
				"country" => $country,
				"latitude" => $latitude,
				"longitude" => $longitude
			);
